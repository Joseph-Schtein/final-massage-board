...
void ariel::Board::post(unsigned int rowIndex, unsigned int columIndex, Direction dir, string const& input){
    ...
    if(rowIndex+input.size() > board.size()){// if the row index is pass the current rows number
        board.resize(rowIndex+input.length()+INCREMENTOR, std::vector<char>(board.at(0).size()+INCREMENTOR, '_'));
        cout << "change rows number " << board.size() << endl; 
    }

    if(columIndex+input.size() > board.at(0).size()){// if the column index is pass the current columns number
        for (unsigned int i = 0 ; i < board.size() ; i++){
            board.at(i).resize(columIndex+input.length()+INCREMENTOR, '_');
        }  
        cout << "change column number " << board.at(0).size() << endl;   
    }

    if(dir == Direction::Horizontal){// entering the string Horizontal
    ...
    if(dir == Direction::Vertical){// entering the string Vertical
        for(auto& index : charInput){
            board.at(rowIndex).at(columIndex) = index;
            rowIndex++;
        }
...
string ariel::Board::read(unsigned int rowIndex, unsigned int columIndex, Direction dir, unsigned int length){
    ...
    if(dir == Direction::Horizontal){ // read the chars in a Horizontal
